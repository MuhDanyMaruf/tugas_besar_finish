...
if (isset($_GET['id'])) {
    if (!empty($_POST)) {
        ...
        // Update the record
        $stmt = $pdo->prepare('UPDATE kontak SET id = ?, nama = ?, email = ?, notelp = ?, pekerjaan = ?, nik = ?, alamat = ?, pinjaman = ? WHERE id = ?');
        $stmt->execute([$id, $nama, $email, $notelp, $pekerjaan, $nik, $alamat, $pinjaman, $_GET['id']]);
...
<div class="content update">
    ...
    <form action="update.php?id=<?= $contact['id'] ?>" method="post">
        ...
        <input type="number" name="nik" value="<?= $contact['nik'] ?>" id="nik">
        <label for="alamat">Alamat</label>
        <label for="pinjaman">Jumlah Pinjaman</label>
        <input type="text" name="alamat" value="<?= $contact['alamat'] ?>" id="alamat">
        <input type="number" name="pinjaman" value="<?= $contact['pinjaman'] ?>" id="pinjaman">
